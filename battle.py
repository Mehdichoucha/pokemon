...
class Battle:
    ...
    def start_battle(self):
        while self.pokemon1.get_pv() > 0 and self.pokemon4.get_pv() > 0:
            ...
            if self.pokemon4.get_pv() <= 0:
                self.capture_pokemon(self.pokemon4)
                break
    ...
    def capture_pokemon(self, defeated_pokemon):
        """Ajoute le Pokémon vaincu au pokédex du joueur."""
        print(f"Tentative de capture de {defeated_pokemon.get_name()}...")

        try:
            with open("pokedex_player.json", "r") as file:
        ...
        pokedex.append({"pokemon": defeated_pokemon.get_name()})
        
        with open("pokedex_player.json", "w") as file:
            json.dump(pokedex, file, indent=4)
        print(f"{defeated_pokemon.get_name()} a été capturé et ajouté au Pokédex !")
        
        print(f"{defeated_pokemon.get_name()} has been captured!")
...
pokemon1 = Pokemon("Pikachu",65,1,75,40,"electric",0)
pokemon2 = Pokemon("Bulbizarre", 45, 1,49, 49, "plant",0)
pokemon3 = Pokemon("Salameche",39,1,52,43,"fire",0)
...
def draw_level_and_up():
    ...
    level_xp_text2 = FONT.render(f"{pokemon3.get_name()}: Level {pokemon3.get_level()}, XP {pokemon3.get_xp()} / {pokemon3.get_xp_to_next_level()}", True, BLACK)
                ...
                if button["rect"].collidepoint(mouse_pos):
                    if button["text"] == "Attack":
                        ...
                        if pokemon3.get_pv() <= 0:
                            ...
                            combat.capture_pokemon(pokemon3)
                            victory_message = f"{pokemon1.get_name()} win! {pokemon3.get_name()} loses!"
                        ...
                        if pokemon1.get_pv() <= 0:
                            pokemon3.gain_experience(350)
                            victory_message = f"{pokemon3.get_name()} win! {pokemon1.get_name()} loses!"
                            xp_message = f"{pokemon3.get_name()} gained 350 XP!"
                            draw_message(victory_message, xp_message)
                            break
